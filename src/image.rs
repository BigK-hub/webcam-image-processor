...
impl Image
{
    ...
    pub fn linear_interpolation(&self, target:&mut Image, colour_one:olc::Pixel,colour_two:olc::Pixel )
    {
        for y in 0..self.height
        {
            for x in 0..self.width
            {
                let factor = (self[(x,y)].brightness(), 255);
                let opposite_fraction = (255-self[(x,y)].brightness(),255);
                // Formula for interpolation (1 - t) * v0 + t * v1 where t is the factor, v0 the startpos and v1 the endpos
                target[(x,y)] = colour_one.clamping_fraction_mul(opposite_fraction).clamping_add(colour_two.clamping_fraction_mul(factor)); 
            }
        }
    }
